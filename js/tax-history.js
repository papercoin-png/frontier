...
async function initializeTaxHistory() {
...
async function getAllTransactions(limit = 1000) {
    return await window.getAll('taxTransactions') || [];
}

async function addTransaction(transaction) {
    const newTx = await window.addTaxTransaction(transaction);
...
async function getTransactionsByType(taxType, limit = 100) {
...
async function getPlayerTaxSummaries() {
    ...
    allTxs.forEach(tx => {
        ...
        if (!summary.lastTransaction || tx.timestamp > summary.lastTransaction) {
            ...
            summary.playerName = tx.playerName;
        }
...
async function getPlayerTaxSummary(playerId) {
...
async function getAllPlayerSummaries() {
    return await getPlayerTaxSummaries();
}

// ===== TAX REPORTS =====
async function generateTaxReport(startDate, endDate, options = {}) {
...
// ===== TAX EXPORTS =====
async function exportTaxData(format = 'json', dateRange = 'all') {
...
// ===== ANALYTICS FUNCTIONS =====
async function getTaxAnalytics() {
...
// ===== CLEANUP =====
async function pruneOldTransactions(daysToKeep = 365) {
...
// ===== EXPOSE TO WINDOW =====
window.HISTORY_STORAGE_KEYS = HISTORY_STORAGE_KEYS;
window.initializeTaxHistory = initializeTaxHistory;
...
window.getPlayerTaxSummaries = getPlayerTaxSummaries;
window.getPlayerTaxSummary = getPlayerTaxSummary;
window.getAllPlayerSummaries = getAllPlayerSummaries;
window.generateTaxReport = generateTaxReport;
window.getTaxReports = getTaxReports;
window.getTaxReport = getTaxReport;
window.getAuditLog = getAuditLog;
window.addAuditEntry = addAuditEntry;
window.exportTaxData = exportTaxData;
window.getTaxExports = getTaxExports;
window.getTaxAnalytics = getTaxAnalytics;
window.getPlayerAnalytics = getPlayerAnalytics;
window.pruneOldTransactions = pruneOldTransactions;
